...
( function( $ ) {
	...
	if (!handyman_services_previewer_vars['need_refresh']) {
		var $style = $('#handyman_services-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="handyman_services-customizer-css"></'+'style'+'>' ).find('#handyman_services-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			handyman_services_set_cookie(handyman_services_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
