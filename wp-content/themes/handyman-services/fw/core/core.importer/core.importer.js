// Handyman Services Importer script
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		HANDYMAN_SERVICES_STORAGE['importer_error_messages'] = '';
		HANDYMAN_SERVICES_STORAGE['importer_ignore_errors'] = true;
		handyman_services_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function handyman_services_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: HANDYMAN_SERVICES_STORAGE['ajax_nonce'],
		action: 'handyman_services_importer_start_import',
	...
	jQuery.post(HANDYMAN_SERVICES_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || HANDYMAN_SERVICES_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				HANDYMAN_SERVICES_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				handyman_services_importer_do_action(steps, idx);
			} else {
				if (HANDYMAN_SERVICES_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + HANDYMAN_SERVICES_STORAGE['importer_error_msg'] + '</h4>' + HANDYMAN_SERVICES_STORAGE['importer_error_messages']);
				} else {
