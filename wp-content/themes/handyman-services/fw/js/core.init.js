...
function handyman_services_ready_actions() {
	...
	// Prepare menus
	if (HANDYMAN_SERVICES_STORAGE['menu_cache']) handyman_services_prepare_menus();
	...
	jQuery('.popup_form.login_form').submit(function(e){
		"use strict";
		var rez = handyman_services_login_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery('.popup_form.registration_form').submit(function(e){
		"use strict";
		var rez = handyman_services_registration_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
	...
	jQuery("form#commentform").submit(function(e) {
		"use strict";
		var rez = handyman_services_comments_validate(jQuery(this));
		if (!rez)
			e.preventDefault();
		return rez;
	});
...
function handyman_services_prepare_menus() {
	"use strict";
	var menus = [
		jQuery('ul#menu_main'),
		jQuery('ul#menu_user'),
...
// Comments form
function handyman_services_comments_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var rules = {
		error_message_text: HANDYMAN_SERVICES_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
		...
		rules: [
			{
				field: 'comment',
				min_length: { value: 1, message: HANDYMAN_SERVICES_STORAGE['strings']['text_empty'] },
				max_length: { value: HANDYMAN_SERVICES_STORAGE['comments_maxlength'], message: HANDYMAN_SERVICES_STORAGE['strings']['text_long']}
			}
		]
	};
	if (form.find('.comments_author input[aria-required="true"]').length > 0) {
		rules.rules.push(
			{
				field: 'author',
				min_length: { value: 1, message: HANDYMAN_SERVICES_STORAGE['strings']['name_empty']},
	...
	if (form.find('.comments_email input[aria-required="true"]').length > 0) {
		rules.rules.push(
			{
				field: 'email',
				min_length: { value: 7, message: HANDYMAN_SERVICES_STORAGE['strings']['email_empty']},
	...
	var error = handyman_services_form_validate(form, rules);
	return !error;
}


// Login form
function handyman_services_login_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var error = handyman_services_form_validate(form, {
		...
		rules: [
			{
				field: "log",
				min_length: { value: 1, message: HANDYMAN_SERVICES_STORAGE['strings']['login_empty'] },
				max_length: { value: 60, message: HANDYMAN_SERVICES_STORAGE['strings']['login_long'] }
			},
			{
				field: "pwd",
				min_length: { value: 4, message: HANDYMAN_SERVICES_STORAGE['strings']['password_empty'] },
				max_length: { value: 30, message: HANDYMAN_SERVICES_STORAGE['strings']['password_long'] }
			}
		]
	});
	if (!error) {
		jQuery.post(HANDYMAN_SERVICES_STORAGE['ajax_url'], {
			...
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html(HANDYMAN_SERVICES_STORAGE['strings']['login_success']);
				setTimeout(function() { 
					location.reload(); 
					}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html(HANDYMAN_SERVICES_STORAGE['strings']['login_failed'] + '<br>' + rez.error);
			}
...
// Registration form 
function handyman_services_registration_validate(form) {
	"use strict";
	form.find('input').removeClass('error_fields_class');
	var error = handyman_services_form_validate(form, {
		error_message_show: true,
		...
		rules: [
			{
				...
				state: { value: 'checked', message: HANDYMAN_SERVICES_STORAGE['strings']['not_agree'] },
			},
			{
			...
			},
			{
				...
				equal_to: { value: 'registration_pwd', message: HANDYMAN_SERVICES_STORAGE['strings']['password_not_equal'] }
			}
		]
	});
	if (!error) {
		jQuery.post(HANDYMAN_SERVICES_STORAGE['ajax_url'], {
			...
			result_box.toggleClass('sc_infobox_style_error', false).toggleClass('sc_infobox_style_success', false);
			if (rez.error === '') {
				result_box.addClass('sc_infobox sc_infobox_style_success').html(HANDYMAN_SERVICES_STORAGE['strings']['registration_success']);
				setTimeout(function() { 
					jQuery('.popup_login_link').trigger('click'); 
					}, 3000);
			} else {
				result_box.addClass('sc_infobox sc_infobox_style_error').html(HANDYMAN_SERVICES_STORAGE['strings']['registration_failed'] + ' ' + rez.error);
			}
			result_box.fadeIn().delay(3000).fadeOut();
		});
	}
