jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(HANDYMAN_SERVICES_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == HANDYMAN_SERVICES_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(HANDYMAN_SERVICES_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				handyman_services_message_success('', HANDYMAN_SERVICES_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(HANDYMAN_SERVICES_STORAGE['strings']['editor_caption_close']);
			} else {
				handyman_services_message_warning(rez.error, HANDYMAN_SERVICES_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		handyman_services_message_confirm(HANDYMAN_SERVICES_STORAGE['strings']['editor_delete_post'], HANDYMAN_SERVICES_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(HANDYMAN_SERVICES_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					handyman_services_message_success('', HANDYMAN_SERVICES_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = HANDYMAN_SERVICES_STORAGE['site_url'];
						}, 1000);
				} else {
					handyman_services_message_warning(rez.error, HANDYMAN_SERVICES_STORAGE['strings']['editor_delete_error']);
				}
			});
