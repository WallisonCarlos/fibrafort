...
if ( !function_exists( 'handyman_services_template_single_portfolio_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_single_portfolio_theme_setup', 1 );
	function handyman_services_template_single_portfolio_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'single-portfolio',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Portfolio item', 'handyman-services'),
...
// Template output
if ( !function_exists( 'handyman_services_template_single_portfolio_output' ) ) {
	function handyman_services_template_single_portfolio_output($post_options, $post_data) {
		...
		handyman_services_template_set_args('prev-next-block', array(
			'post_options' => $post_options,
			'post_data' => $post_data
		));
		get_template_part(handyman_services_get_file_slug('templates/_parts/prev-next-block.php'));
		...
		if ($post_data['post_protected']) { 
			...
			if (handyman_services_get_custom_option('show_post_tags')=='yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				?>
				<div class="post_info">
					<span class="post_info_item post_info_tags"><?php esc_html_e('in', 'handyman-services'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php
			} 
		}
