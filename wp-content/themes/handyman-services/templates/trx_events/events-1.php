...
if ( !function_exists( 'handyman_services_template_events_1_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_events_1_theme_setup', 1 );
	function handyman_services_template_events_1_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'handyman-services'),
...
// Template output
if ( !function_exists( 'handyman_services_template_events_1_output' ) ) {
	function handyman_services_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(handyman_services_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: handyman_services_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
