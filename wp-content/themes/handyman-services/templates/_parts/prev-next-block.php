<?php
// Get template args
extract(handyman_services_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = handyman_services_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			...
			$args = handyman_services_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = handyman_services_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = handyman_services_strshort($next->post_title, 30);
				?>
