...
if ( !function_exists( 'handyman_services_template_single_matches_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_single_matches_theme_setup', 1 );
	function handyman_services_template_single_matches_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'single-matches',
			'mode'   => 'matches',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single matches', 'handyman-services'),
...
// Template output
if ( !function_exists( 'handyman_services_template_single_matches_output' ) ) {
	function handyman_services_template_single_matches_output($post_options, $post_data) {
		...
		if (!$post_data['post_protected']) {
			...
			get_template_part(handyman_services_get_file_slug('templates/trx_matches/_parts/match-info.php'));
		}
		...
		?>
			<div class="match_preview"><a href="<?php echo esc_url($match_preview); ?>"><?php echo esc_html__("Go to the announcement &#8594;", 'handyman-services'); ?></a></div>
		<?php
		} 
