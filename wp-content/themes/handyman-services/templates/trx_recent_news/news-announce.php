...
if ( !function_exists( 'handyman_services_template_news_announce_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_news_announce_theme_setup', 1 );
	function handyman_services_template_news_announce_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'handyman-services')
		));

		// Add thumb sizes into list
		handyman_services_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		handyman_services_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 770, 'h' => 434 ) );
		handyman_services_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 770, 'h' => 217 ) );
		handyman_services_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'handyman_services_template_news_announce_output' ) ) {
	function handyman_services_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = handyman_services_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && handyman_services_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);
