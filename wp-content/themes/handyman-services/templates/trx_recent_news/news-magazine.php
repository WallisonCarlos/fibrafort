...
if ( !function_exists( 'handyman_services_template_news_magazine_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_news_magazine_theme_setup', 1 );
	function handyman_services_template_news_magazine_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'handyman-services'),
...
// Template output
if ( !function_exists( 'handyman_services_template_news_magazine_output' ) ) {
	function handyman_services_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(handyman_services_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//handyman_services_get_theme_option('blog_counters');
