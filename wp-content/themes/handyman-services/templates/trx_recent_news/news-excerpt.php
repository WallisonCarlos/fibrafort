...
if ( !function_exists( 'handyman_services_template_news_excerpt_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_news_excerpt_theme_setup', 1 );
	function handyman_services_template_news_excerpt_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'handyman-services'),
...
// Template output
if ( !function_exists( 'handyman_services_template_news_excerpt_output' ) ) {
	function handyman_services_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(handyman_services_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: handyman_services_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
