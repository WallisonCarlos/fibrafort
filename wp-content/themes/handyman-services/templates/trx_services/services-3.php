...
if ( !function_exists( 'handyman_services_template_services_3_theme_setup' ) ) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_template_services_3_theme_setup', 1 );
	function handyman_services_template_services_3_theme_setup() {
		handyman_services_add_template(array(
			'layout' => 'services-3',
			'template' => 'services-3',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 3/', 'handyman-services')
		));
	}
}

// Template output
if ( !function_exists( 'handyman_services_template_services_3_output' ) ) {
	function handyman_services_template_services_3_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					echo trim(handyman_services_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
				} else {
					?>
                    ...
                    if ($show_title) {
                        if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
                            ?><h5 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php echo trim($post_data['post_title']); ?></a></h5><?php
                        } else {
                            ?><h5 class="sc_services_item_title"><?php echo trim($post_data['post_title']); ?></h5><?php
                        }
                    }
