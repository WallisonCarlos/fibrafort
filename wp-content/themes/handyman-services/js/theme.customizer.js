...
function handyman_services_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = HANDYMAN_SERVICES_STORAGE['co_add_params']!=undefined ? HANDYMAN_SERVICES_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#handyman_services-customizer-styles-'+option).length > 0 ? jQuery('#handyman_services-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="handyman_services-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
