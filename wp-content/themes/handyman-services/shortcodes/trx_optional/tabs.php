...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('handyman_services_sc_tabs_theme_setup')) {
	add_action( 'handyman_services_action_before_init_theme', 'handyman_services_sc_tabs_theme_setup' );
	function handyman_services_sc_tabs_theme_setup() {
		add_action('handyman_services_action_shortcodes_list', 		'handyman_services_sc_tabs_reg_shortcodes');
		if (function_exists('handyman_services_exists_visual_composer') && handyman_services_exists_visual_composer())
			add_action('handyman_services_action_shortcodes_list_vc','handyman_services_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('handyman_services_sc_tabs')) {	
	function handyman_services_sc_tabs($atts, $content = null) {
		...
		handyman_services_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = handyman_services_storage_get_array('sc_tab_data', 'titles');
		...
		handyman_services_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
		handyman_services_enqueue_script('jquery-effects-fade', false, array('jquery','jquery-effects-core'), null, true);
	
		$tabs_output .= $titles_output
			. '</ul>' 
			. ($content)
			.'</div>';
		return apply_filters('handyman_services_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	handyman_services_require_shortcode("trx_tabs", "handyman_services_sc_tabs");
}


if (!function_exists('handyman_services_sc_tab')) {	
	function handyman_services_sc_tab($atts, $content = null) {
		if (handyman_services_in_shortcode_blogger()) return '';
		extract(handyman_services_html_decode(shortcode_atts(array(
			// Individual params
		...
		handyman_services_storage_inc_array('sc_tab_data', 'counter');
		$counter = handyman_services_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : handyman_services_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = handyman_services_storage_get_array('sc_tab_data', 'titles');
		...
		handyman_services_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (handyman_services_param_is_on(handyman_services_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(handyman_services_storage_get_array('sc_tab_data', 'height') != '' ? handyman_services_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (handyman_services_param_is_on(handyman_services_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('handyman_services_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	handyman_services_require_shortcode("trx_tab", "handyman_services_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'handyman_services_sc_tabs_reg_shortcodes' ) ) {
	//add_action('handyman_services_action_shortcodes_list', 'handyman_services_sc_tabs_reg_shortcodes');
	function handyman_services_sc_tabs_reg_shortcodes() {
	
		handyman_services_sc_map("trx_tabs", array(
			"title" => esc_html__("Tabs", 'handyman-services'),
			"desc" => wp_kses_data( __("Insert tabs in your page (post)", 'handyman-services') ),
			"decorate" => true,
			"container" => false,
			"params" => array(
				"style" => array(
					"title" => esc_html__("Tabs style", 'handyman-services'),
					"desc" => wp_kses_data( __("Select style for tabs items", 'handyman-services') ),
					"value" => 1,
					"options" => handyman_services_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
					"title" => esc_html__("Initially opened tab", 'handyman-services'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'handyman-services') ),
					"divider" => true,
					"value" => 1,
					"min" => 0,
					"type" => "spinner"
				...
				"scroll" => array(
					...
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'handyman-services') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
			...
			"children" => array(
				"name" => "trx_tab",
				"title" => esc_html__("Tab", 'handyman-services'),
				"desc" => wp_kses_data( __("Tab item", 'handyman-services') ),
				"container" => true,
				"params" => array(
					"title" => array(
						"title" => esc_html__("Tab title", 'handyman-services'),
						"desc" => wp_kses_data( __("Current tab title", 'handyman-services') ),
						"value" => "",
						"type" => "text"
					),
					"_content_" => array(
						"title" => esc_html__("Tab content", 'handyman-services'),
						"desc" => wp_kses_data( __("Current tab content", 'handyman-services') ),
						"divider" => true,
						"rows" => 4,
						"value" => "",
						"type" => "textarea"
...
-------------------------------------------------------------------- */
if ( !function_exists( 'handyman_services_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('handyman_services_action_shortcodes_list_vc', 'handyman_services_sc_tabs_reg_shortcodes_vc');
	function handyman_services_sc_tabs_reg_shortcodes_vc() {
	
		$tab_id_1 = 'sc_tab_'.time() . '_1_' . rand( 0, 100 );
		$tab_id_2 = 'sc_tab_'.time() . '_2_' . rand( 0, 100 );
		vc_map( array(
			"base" => "trx_tabs",
			"name" => esc_html__("Tabs", 'handyman-services'),
			"desc" => wp_kses_data( __("Tabs", 'handyman-services') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Tabs style", 'handyman-services'),
					"desc" => wp_kses_data( __("Select style of tabs items", 'handyman-services') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "initial",
					"heading" => esc_html__("Initially opened tab", 'handyman-services'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'handyman-services') ),
					"class" => "",
					"value" => 1,
					"type" => "textfield"
				...
				array(
					"param_name" => "scroll",
					"heading" => esc_html__("Scroller", 'handyman-services'),
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'handyman-services') ),
					"class" => "",
			...
			'default_content' => '
				[trx_tab title="' . esc_html__( 'Tab 1', 'handyman-services' ) . '" tab_id="'.esc_attr($tab_id_1).'"][/trx_tab]
				[trx_tab title="' . esc_html__( 'Tab 2', 'handyman-services' ) . '" tab_id="'.esc_attr($tab_id_2).'"][/trx_tab]
			',
			"custom_markup" => '
		...
		vc_map( array(
			"base" => "trx_tab",
			"name" => esc_html__("Tab item", 'handyman-services'),
			"desc" => wp_kses_data( __("Single tab item", 'handyman-services') ),
			...
			"params" => array(
				array(
					"param_name" => "title",
					"heading" => esc_html__("Tab title", 'handyman-services'),
					"desc" => wp_kses_data( __("Title for current tab", 'handyman-services') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "tab_id",
					"heading" => esc_html__("Tab ID", 'handyman-services'),
					"desc" => wp_kses_data( __("ID for current tab (required). Please, start it from letter.", 'handyman-services') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Tabs extends HANDYMAN_SERVICES_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends HANDYMAN_SERVICES_VC_ShortCodeTab {}
	}
}
?>